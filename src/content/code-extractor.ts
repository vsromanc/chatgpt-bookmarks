...
export function detectCodeLanguage(preElement: HTMLPreElement): string {
    ...
    if (container) {
        const header = container.querySelector('.flex.items-center.text-token-text-secondary.px-4.py-2.text-xs.font-sans');
        if (header) {
    ...
    const preClassLang = Array.from(preElement.classList)
        .find(c => c.startsWith('language-'));
    if (preClassLang) return preClassLang.split('-')[1];

    const codeElement = preElement.querySelector('code');
    const codeClassLang = Array.from(codeElement?.classList || [])
        .find(c => c.startsWith('language-'));
